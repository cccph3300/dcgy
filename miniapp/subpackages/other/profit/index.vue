  ...
  <view class="page profit-page">
    <view class="profit-head">
      <view>
        ...
        <view class="head-title">利润统计</view>
      </view>
    ...
    <view v-if="loading" class="soft-card empty">正在计算利润...</view>
      ...
      <template v-if="detailMode === 'goods'">
        <view v-for="item in goodsRows" :key="item.key" class="profit-row">
          <view class="row-main">
            ...
            <text class="muted">{{ item.quantityText }} · 成本¥{{ money(item.costAmount) }}</text>
          </view>
          <view class="row-money">
            <text>售¥{{ money(item.salesAmount) }}</text>
            <text :class="{ loss: item.profitAmount < 0 }">利¥{{ money(item.profitAmount) }}</text>
          </view>
        </view>
        <view v-if="!goodsRows.length" class="empty">当前范围没有已售订单</view>
      </template>

      <template v-else>
        <view v-for="item in orderRows" :key="item.key" class="profit-row order-profit-row" @click="openOrderProfit(item)">
          <view class="row-main">
...
export default {
  ...
  methods: {
    ...
    openOrderProfit(item) {
      if (item.source === 'supermarket') {
        uni.navigateTo({ url: `/subpackages/delivery/detail?id=${item.id}&profit=1` })
      } else {
        uni.navigateTo({ url: `/pages/orders/detail?id=${item.id}&profit=1` })
      }
    },
    async loadProfit() {
      this.loading = true
      ...
      try {
        const result = await request({
          url: `/api/profit?mode=${encodeURIComponent(this.mode)}&day=${encodeURIComponent(this.day)}&startDate=${encodeURIComponent(this.startDate)}&endDate=${encodeURIComponent(this.endDate)}`
        })
